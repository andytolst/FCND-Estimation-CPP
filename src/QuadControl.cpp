...
VehicleCommand QuadControl::GenerateMotorCommands(float collThrustCmd, V3F momentCmd)
{
  ...
  // note, for some reason we have to negate kappa, otherwise yaw rate doens't stabilize
  float q3 = momentCmd.z / -kappa;
  float q4 = collThrustCmd;

  float f_1 = ( q1 + q2 + q3 + q4) / 4.0;
  float f_2 = (-q1 + q2 - q3 + q4) / 4.0;
  float f_3 = (-q1 - q2 + q3 + q4) / 4.0;
  float f_4 = ( q1 - q2 - q3 + q4) / 4.0;

  cmd.desiredThrustsN[0] = f_1;
  cmd.desiredThrustsN[1] = f_2;
  cmd.desiredThrustsN[2] = f_4; // <--- nice catch
...
V3F QuadControl::RollPitchControl(V3F accelCmd, Quaternion<float> attitude, float collThrustCmd)
{
  ...
  float b_x_commanded_dot = kpBank * b_x_err;

  float b_y = R(1,2);
  float b_y_err = b_y_c - b_y;
  float b_y_commanded_dot = kpBank * b_y_err;
...
float QuadControl::AltitudeControl(float posZCmd, float velZCmd, float posZ, float velZ, Quaternion<float> attitude, float accelZCmd, float dt)
{
  ...
  float b_z = R(2,2);
  float u_1_bar = kpPosZ * (posZCmd - posZ) + kpVelZ * (velZCmd - velZ) + KiPosZ * integratedAltitudeError + accelZCmd;

  thrust = -mass * (u_1_bar - CONST_GRAVITY) / b_z;
